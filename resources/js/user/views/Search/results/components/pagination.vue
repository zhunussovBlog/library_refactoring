	...
	<div id="pagination" class="d-flex">
		<div class="d-flex align-items-center bg-lightgrey rounded px-3 mr-3 mt-1">
			...
			<div class="position-relative d-flex align-items-center text-grey mx-2">
				...
				<div class="wrapper" v-for="(page,index) in pages" :key="index" @click="getResults(page);">{{page}}</div>
			</div>
		...
		<div class="d-flex align-items-center bg-lightgrey rounded px-3 mt-1">
			...
			<form @submit.prevent="getResults(inputPage);" class="d-flex align-items-center">
				<input class="rounded border border-darkgrey text-darkgrey" type="text" v-model="inputPage"/>
				&nbsp;
				&nbsp;
				<button type="submit" class="text-black"><right-long /></button>
			</form>
		</div>
	</div>
</template>
...
<template>
	<div id="pagination" class="d-flex">
		<div class="d-flex align-items-center bg-lightgrey rounded px-3 mr-3 mt-1">
			<div>
				<right-little class="rotate"/>
			</div>
			<div class="position-relative d-flex align-items-center text-grey mx-2">
				<div class="wrapper wrapperRect transition" />
				<div class="wrapper" v-for="(page,index) in pages" :key="index" >{{page}}</div>
			</div>
			<div>
				<right-little/>
			</div>
		...
		<div class="d-flex align-items-center bg-lightgrey rounded px-3 mt-1">
			...
			<form  class="d-flex align-items-center ml-2">
				<input class="rounded border border-darkgrey text-darkgrey" type="text" v-model="inputPage"/>

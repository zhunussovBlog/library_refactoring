<template>
	<div id="sidebar" class="transition color-white overflow-auto">
		<a href="/" class="logo align-items-center justify-content-center cursor-pointer">
		...
		<div class="mt-10">
			...
			<div class="col" v-for="(tab,index) in tabs" :key="index">
				<!-- parent route -->
				<div class="title cursor-pointer align-items-center justify-content-between" @click="showTab(tab)">
					...
					<ArrowDown class="transition color-white" :class="{'rotate':tab.shown}" v-if="!tab.noChildren"/>
				</div>
				<!-- children routes. Presented only if there are children on the current tab -->
				<div class="hidden transition mt-5" :class="{'shown':tab.shown}" v-if="!tab.noChildren">
					<!-- for loop for children -->
					<!-- children is a method because we cannot get children using routes -->
					<div class="link transition justify-content-between" :class="{seen:currentRoute.name==link.name}" v-for="(link,i) in children(tab.name)" :key="i" @click="goTo(link.name)">
						{{$tc(link.name,10)}}
				...
				<div class="sidebar_line mt-10" v-if="index!=tabs.length-1"/>
			</div>
		</div>
	</div>
</template>

<template>
	<div class="relative" tabindex="1" @focusout="shown=false">
		<div class="align-items-center" @click="showDropdown()">
			<span class="regIcon">
				<RegUser/>
				&nbsp;
			</span>
		...
		<div class="dropdown dropdown-right transition border-radius bg-white shadowed pl-18" :class="{shown:shown}">
			<div class="link border-bottom" v-for="(link,index) in links" :key="index" @click="link.method">{{$t(link.name)}}</div>
		</div>
	</div>
</template>
